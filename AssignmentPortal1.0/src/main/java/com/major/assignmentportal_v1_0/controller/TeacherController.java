...
public class TeacherController extends Teacher {
    ...
    public String login(@ModelAttribute("teacher") Teacher teacher, Model model_id){
        BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
        if(encoder.matches(teacher.getTch_password(), teacherRepo.findById(teacher.getTch_id()).orElse(teacher).getTch_password())){
            ...
            model_id.addAttribute("teacher_id", teacher.getTch_id());
    ...
    public String loadAssignmentForm(Model model){
    ...
    public String assign(@ModelAttribute("teacherAssignment") TeacherAssignment teacherAssignment, @RequestParam("file")MultipartFile assignment_file) throws IOException {
        ...
        if(teacherRepo.findById(teacherAssignment.getTch_id()).orElse(new Teacher()).getSubject_name().equals(teacherAssignment.getSubject_name())){
    ...
    public String view(Model model){
        ...
        for(int i = 0; i < teacherAssignmentList.size(); i ++)
            System.out.println(teacherAssignmentList.get(i).getTch_id());
        model.addAttribute("teacherAssignmentList", teacherAssignmentList);
